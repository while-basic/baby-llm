...
#Description: Q-Learning module for neural child development
#Version:    1.0.0
#License:    MIT
...
Extracted from neural-child-init/q_learning.py
...
class QNetwork(nn.Module):
    ...
    def update(self) -> float:
        ...
        with torch.no_grad():
            ...
            target_q_values = (
                rewards + (1 - next_q_values.detach()) * self.gamma * next_q_values
            )
...
class QLearningSystem:
    ...
    def __init__(
        ...
        Args:
            ...
            action_dim: Dimension of action space
            hidden_dim: Hidden layer dimension
            device: Device to use for computation (string or torch.device)
        """
    ...
    def train_episode(
        self,
        get_state_fn: Callable[[], torch.Tensor],
        get_reward_fn: Callable[[], float],
        step_env_fn: Callable[[int], torch.Tensor],
        max_steps: int = 1000
    ) -> Dict[str, float]:
