...
#Description: Base neural network class for the mind simulation
#Version:    1.0.0
#License:    MIT
...
Extracted from neural-child-4/neuralchild/core/neural_network.py
Adapted imports to use unified schemas.
"""
...
class GrowthMetrics(BaseModel):
    ...
    pruning_rate: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Rate at which unused connections are removed"
    )
    ...
    integration: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Degree of integration with other networks"
    )
    ...
    @field_validator(
        'connection_density', 'plasticity', 'pruning_rate',
        'specialization', 'integration', 'adaptability'
    )
    @classmethod
...
class NeuralNetwork(nn.Module, ABC):
    ...
    def _check_for_network_growth(self, trigger: str) -> bool:
        ...
        if (not growth_occurred and prune_candidates and
                random.random() < self.growth_metrics.pruning_rate):
    ...
    def _grow_network_for_new_stage(self, stage: DevelopmentalStage) -> None:
        ...
        logger.info(
            f"Network {self.name} advancing to stage {stage.name} - "
            "Structure changes should be implemented in subclass"
        )
    ...
    def save_model(
        self,
        path: str,
        format: Literal["pytorch", "torchscript", "onnx"] = "pytorch"
    ) -> bool:
        ...
        try:
            ...
            if 'input_dim' in checkpoint and 'output_dim' in checkpoint:
                if (checkpoint['input_dim'] != self.input_dim or
                    ...
                    logger.warning(
                        f"Dimension mismatch: saved model has dimensions "
                        f"({checkpoint['input_dim']}, {checkpoint['output_dim']}), "
                        f"but current model has ({self.input_dim}, {self.output_dim}). "
                        "Attempting to adapt."
                    )
        ...
        with torch.no_grad():
            for name in common_params:
                ...
                if self_param.shape != other_param.shape:
                    logger.warning(
                        f"Parameter {name} has mismatched shape: "
                        f"{self_param.shape} vs {other_param.shape}"
                    )
                    continue
